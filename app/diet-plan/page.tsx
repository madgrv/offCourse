            ...
            {days.map((day) => (
              <TabsContent key={day} value={day}>
                ...
                Object.keys(dietPlan.days[day].meals).length > 0 ? (
                  Object.entries(dietPlan.days[day].meals).map(
                    ([mealType, meal]) => (
                      <MealCard
                        key={mealType}
                        title={mealType}
                        ...
                        isLoading={{
                          save: isProcessing[`save-${day}-${mealType}`] || false,
                          mealComplete: isProcessing[`meal-complete-${day}-${mealType}`] || false,
                          foodComplete: Object.keys(isProcessing).some(key => 
                            key.startsWith(`food-complete-${day}-${mealType}`))
                        }}
                        errors={{
                          save: actionErrors[`save-${day}-${mealType}`],
                          mealComplete: actionErrors[`meal-complete-${day}-${mealType}`],
                          foodComplete: Object.entries(actionErrors)
                            .filter(([key]) => key.startsWith(`food-complete-${day}-${mealType}`))
                            .map(([_, value]) => value)
                            .filter(Boolean)[0]
                        }}
